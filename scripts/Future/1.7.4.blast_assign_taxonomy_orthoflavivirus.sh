#!/bin/bash
#SBATCH --job-name=1.7.4.blast_assign_taxonomy_whole_genome
...
# Define paths
query_file="/home/people/23203786/scratch/Nelson-Dissertation/results/fasta/orthoflavivirus_sequences.fasta"
base_output_dir="/home/people/23203786/scratch/Nelson-Dissertation/results/blast_output/orthoflavivirus"
...
for serotype in "${!serotypes[@]}"; do
    ...
    blastx -db $db_path -query $query_file -out $blast_output -evalue 1e-5 -outfmt "6 qseqid qlen sseqid pident length qstart qend sstart send evalue bitscore staxids" -num_threads 10
    ...
    python3 $taxonomy_script --blast_flavor blastx --cutoff_species 70 --cutoff_family 60 --cutoff_phylum 50 --output_dir $base_output_dir --verbose --blast_file $blast_output -- $query_file $taxonomy_file
    if [ $? -ne 0 ]; then
