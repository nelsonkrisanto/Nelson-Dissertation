...
def generate_semi_nested_primer_combinations(mapping_file, metadata_file, outer_min_len, inner_max_len):
    try:
        ...
        for combo in first_round_combinations.itertuples(index=False):
            ...
            new_fwd_combinations = find_combinations(specific_fwd, rev_primers, 0, inner_max_len, 0, 'second')
            new_rev_combinations = find_combinations(fwd_primers, specific_rev, 0, inner_max_len, 0, 'second')
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        ...
        generate_semi_nested_primer_combinations(mapping_file, metadata_file, int(outer_min_len), int(inner_max_len))
