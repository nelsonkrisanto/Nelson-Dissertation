...
outer_primers="/home/people/23203786/scratch/Nelson-Dissertation/results/tsv/semi_nested_outer_primer_combinations.tsv"
inner_primers="/home/people/23203786/scratch/Nelson-Dissertation/results/tsv/semi_nested_inner_primer_combinations.tsv"
...
perl "$tool" -s "$sequences" -p "$outer_primers" -m 2 -i -e -r > semi_nested_outer_insilico_PCR_results.txt 2> semi_nested_outer_insilico_PCR_amplicons.fasta

# Check if the outer PCR results were successfully generated
if [ $? -eq 0 ]; then
  # Use the outer amplicons as input for the second round
  perl "$tool" -s semi_nested_outer_insilico_PCR_amplicons.fasta -p "$inner_primers" -m 2 -i -e -r > semi_nested_inner_insilico_PCR_results.txt 2> semi_nested_inner_insilico_PCR_amplicons.fasta
